...
class YOLOv3Net(nn.Module):
    ...
    def setFreezeBackbone(self, freeze):
        if isinstance(freeze, bool):
            rg = (not freeze)
            for p in self.layer_1.parameters():
                p.requires_grad = rg
            for p in self.layer_2.parameters():
                p.requires_grad = rg
            for p in self.layer_3.parameters():
                p.requires_grad = rg
            for p in self.layer_4.parameters():
                p.requires_grad = rg
            for p in self.layer_5.parameters():
                p.requires_grad = rg

    def isBackboneFreezed(self):
        freezed = False
        for p in self.layer_1.parameters():
            freezed = not p.requires_grad
            break
        return freezed
